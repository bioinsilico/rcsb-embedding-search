...
from dataset.tm_score_masked_from_embeddings_dataset import TmScoreMaskedFromEmbeddingsDataset, collate_masked_fn
from dataset.utils.custom_weighted_random_sampler import CustomWeightedRandomSampler
from dataset.utils.tm_score_weight import fraction_score_of, tm_score_weights
from lightning_module.training.embedding_training import LitEmbeddingMaskedTraining
...
def main(cfg: TrainingConfig):
    ...
    training_set = TmScoreMaskedFromEmbeddingsDataset(
        tm_score_file=cfg.training_set.tm_score_file,
        embedding_path=cfg.training_set.data_path,
        neighbor_path=cfg.training_set.neighbor_path,
    ...
    validation_set = TmScoreMaskedFromEmbeddingsDataset(
        tm_score_file=cfg.validation_set.tm_score_file,
        embedding_path=cfg.validation_set.data_path,
        neighbor_path=cfg.validation_set.neighbor_path
    )
    ...
    model = LitEmbeddingMaskedTraining(
        nn_model=nn_model,
    ...
    checkpoint_callback = L.pytorch.callbacks.ModelCheckpoint(
        monitor=LitEmbeddingMaskedTraining.PR_AUC_METRIC_NAME,
        mode='max',
        filename='{epoch}-{' + LitEmbeddingMaskedTraining.PR_AUC_METRIC_NAME + ':.2f}'
    )
